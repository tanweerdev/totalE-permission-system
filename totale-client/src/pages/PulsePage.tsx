...
import type { Facility, PulseRow } from '../types';
...
export default function PulsePage() {
  ...
  const [data, setData] = useState<PulseRow[]>([]);
  ...
  async function fetchData() {
    ...
    setError(null);
    try {
      const rows = await analyticsApi.getPulse({
        dateFrom,
        dateTo,
  ...
  return (
    <div className="p-6 space-y-5">
      <h2 className="text-lg font-semibold text-white">Pulse Analytics</h2>
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-2 text-gray-300">{row.category}</td>
                  <td className="px-4 py-2 text-right text-white font-medium">{Number(row.avgScore).toFixed(2)}</td>
                  <td className="px-4 py-2 text-right text-gray-400">{row.responseCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
